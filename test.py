                ...
                if det is not None and len(det):
                    ...
                    if len(outputs) > 0:
                        ...
                        for j, (output, conf) in enumerate(zip(outputs, confs)):
                            coor_dict[output[4]] = [output[2]*0.5+output[0]*0.5, (output[1] + output[3]) / 2]
                    # print(len(coor_dict))
...
if __name__ == '__main__':
    ...
    str_video = ['ele', 'en', 'in']
    skip_frame_test = [3, 4, 5]
    second_test = [30, 60]#[15, 20, 30, 60]
    limit_test = [4, 2] #[8, 6, 4, 2]
    resolution_test = ['640']
    video_test = ['1,2']#'0,1', '1,2', '0,2']
            ...
            for sec in range(len(second_test)):
                for resol_test in resolution_test:
                    for vi_test in video_test:
                        args.heatmapsec = 120
                        args.video = vi_test
                        args.second = second_test[sec]
                        args.limit = limit_test[sec]
                        args.frame = skip_test
                        args.resolution = resol_test
                        if args.realtime == 0 and args.video != 'None':
                            x = args.video.split(',')
                            y = args.resolution.split(',')
                            video1 = videos[int(x[0])]
                            if args.num_video == 2:
                                video2 = videos[int(x[1])]
                            args.matrix = 'coor_' + str_video[int(x[0])] + '_' + y[0]
                            if args.num_video == 2:
                                args.matrix += ' coor_' + str_video[int(x[1])] + '_' + y[0]

                        start_time = time.time()
                        frame_get1 = Manager().Queue()
                        frame_get2 = Manager().Queue()
                        print('{:5} {:5} {:4} {:3} {:5}'.format(str_video[int(x[0])], str_video[int(x[1])],
                                                                  args.frame, args.second, args.resolution))
                        p1 = Process(target=fg.get_frame_video, args=(video1, frame_get1, args.frame,
                                                                      args.second, args.fps, args.resolution,
                                                                      args.limit))
                        p1.start()
                        if args.num_video == 2:
                            p2 = Process(target=fg.get_frame_video, args=(video2, frame_get2, args.frame,
                                                                          args.second, args.fps, args.resolution,
                                                                          args.limit))
                            p2.start()

                        """
                        limit = frame_get1.qsize()
                        if args.num_video == 2:
                            size2 = frame_get2.qsize()
                            if limit > size2:
                                limit = size2
                        if args.limit == 0 or args.limit > limit:
                            args.limit = limit
                        """
                        ...
                        if args.num_video == 2:
                            p6.join()
                        p7.join()
                        test_output.write('{:5} {:5} {:4} {:3} {:5} {:.3f}\n'.format(str_video[int(x[0])], str_video[int(x[1])], args.frame, args.second, args.resolution, time.time() - start_time))
                        print('{:5} {:5} {:4} {:3} {:5} {:.3f}\n'.format(str_video[int(x[0])], str_video[int(x[1])], args.frame, args.second, args.resolution, time.time() - start_time))
    test_output.close()
