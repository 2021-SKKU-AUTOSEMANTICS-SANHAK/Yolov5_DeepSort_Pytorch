...
cap = cv2.VideoCapture("../../test_video/Single1.mp4")
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        while True:
            ...
            for k, j in enumerate(boxes[0][:i]):
                if scores[k] > 0.2:
                    ymin = int(j[0] * width + 20)
                    xmin = int(j[1] * height + 20)
                    ymax = int(j[2] * width)
                    xmax = int(j[3] * height)
                    print(str(j[0]) + ' : ' + str(ymin))
            ...
            ims = cv2.resize(img, (500, 1000))
            cv2.imshow("object detection", ims)
